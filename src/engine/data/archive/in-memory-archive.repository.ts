...
export class InMemoryArchiveRepository implements IArchiveRepository {
  ...
  async save(record: ArchiveRecord): Promise<void> {
    ...
    if (this.byContentHash.has(record.contentSha256)) {
      throw new Error(
        `Archive content hash already exists: ${record.contentSha256}`,
      );
    }
    const frozen = cloneRecord(record);
    this.byId.set(frozen.archiveId, frozen);
    this.byHash.set(frozen.sha256.toLowerCase(), frozen.archiveId);
    this.byContentHash.set(
      frozen.contentSha256.toLowerCase(),
      frozen.archiveId,
    );
  }
  ...
  async findByContentHash(contentSha256: string): Promise<ArchiveRecord | null> {
    const id = this.byContentHash.get(contentSha256.toLowerCase());
    if (!id) {
      return null;
    }
    return this.byId.get(id) ?? null;
