...
type SuggestionWithRelated = OrthographySuggestionView & { relatedWords?: readonly string[] };
...
function rangesForText(text: string, suggestions: OrthographySuggestionView[]): SuggestionWithRelated[] {
  return suggestions.filter((item) => item.start >= 0 && item.end > item.start && item.start < text.length && item.end <= text.length).sort((a, b) => a.start - b.start || b.end - a.end) as SuggestionWithRelated[];
}
...
export function SpellcheckTextarea({ value, suggestions, placeholder, rows = 4, disabled, onChange, onKeyDown, inputRef, className }: Props) {
  ...
  function apply(word: string) {
    if (!active) return;
    onChange(`${value.slice(0, active.start)}${word}${value.slice(active.end)}`);
    setActiveIndex(null);
  }
    ...
    <div className="relative">
      ...
      {active ? (
        <div className="absolute left-3 right-3 top-2 z-30 rounded-lg border border-[#0B1F3A]/15 bg-white p-2 shadow-xl" role="dialog" aria-label={`${active.sourceWord} үгийн санал`}>
          <div className="flex items-center justify-between gap-2"><p className="text-xs"><span className="font-semibold text-[#991B1B]">{active.sourceWord}</span><span className="mx-1 text-[#8A939D]">→</span><span className="font-semibold text-[#1B5E20]">{active.suggestedWord}</span></p><button type="button" className="text-[11px] text-[#66717D]" onClick={() => setActiveIndex(null)}>Хаах</button></div>
