...
const COMMON_TYPO_CORRECTIONS: Record<string, string> = {
  ...
  хувцаса: "хувцас", явч: "яавч", явач: "яавч",
};
...
for (const stem of ["гэр", "хүн", "хувцас", "хууль", "хэрэг", "баримт", "эрх", "асуудал", "шүүх", "гэрээ", "ажил", "бичиг", "асуулт", "хариулт", "шинжилгээ"] as const) for (const suffix of ["", "т", "д", "аас", "ээс", "руу", "тай", "тэй", "ийн", "ын", "ийг", "ыг"]) DICTIONARY.add(normalizeMongolianWord(`${stem}${suffix}`));
...
export function suggestDictionaryWords(word: string, limit = 5, options?: { highConfidenceOnly?: boolean }): readonly string[] {
  ...
  for (const candidate of DICTIONARY) { if (Math.abs(candidate.length - normalized.length) > maxDistance + 1) continue; const distance = spellingDistance(normalized, candidate); if (distance <= maxDistance) scored.push({ word: candidate, distance }); }
...
/** Related-word search: exact root first, then a small root-distance fallback for misspelled roots. */
